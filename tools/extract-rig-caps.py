...
def extract_bands(props: dict[str, str]) -> list[dict]:
    """Walk Rig/Bands\\N\\{Num,Name,Start,End} into [{num, name, start, end}].

    A .rig carries one row per ITU region wherever the band edges differ
    (40 m is 7.0-7.2 MHz in region 1, 7.0-7.3 in region 2). The SPA never
    picks a region — it only needs "where does this band start" and "which
    ...
    return [folded[n] for n in sorted(folded)]
...
def main() -> int:
    ...
    for p in sorted(RIGS_DIR.glob("*.rig")):
        ...
        scope_modes = extract_num_name_list(props, "ScopeModes")
        rigs.append((civ, model, meters, cmds, caps, inputs, preamps, attenuators, antennas, bands, scope_modes, mem_format))
    ...
    lines = [
        ...
        "//",
        "// Each entry: civAddr -> { model, caps, meters, cmds, inputs, preamps, attenuators, antennas, bands, scopeModes, memFormat }",
        ...
        "//   scopeModes: [{num, name}, ...] — the rig's scope modes (0x27 0x14:",
        "//           0 Center, 1 Fixed, 2 Scroll-C, 3 Scroll-F); the SPA shows keys",
        "//           for the ones it renders.",
    ...
    for civ, model, meters, cmds, caps, inputs, preamps, attenuators, antennas, bands, scope_modes, mem_format in rigs:
        lines.append(
            ...
            f"scopeModes: {js_num_name_list(scope_modes)}, "
